...
export default function GrabMart() {
  const [grabmarts, setGrabmarts] = useState([]);
  ...
  const fetchGrabMarts = async () => {
    setLoading(true);
    try {
      const result = await getStores(currentPage, totalData);
      setGrabmarts(result.data);
      ...
      setLoading(false);
      toast("Fetch Failed", {
        description: "Unable to load GrabMarts. Please try again later.",
      });
    }
  };
  ...
  const handleDelete = async (id) => {
    const confirmDelete = await confirm({
      title: "Are you sure?",
      description: "This will permanently delete the GrabMart.",
    });
    if (!confirmDelete) return;

    try {
      await deleteStore(id);
      await fetchGrabMarts();

      toast("GrabMart Deleted", {
        description: "The GrabMart has been successfully deleted.",
      });
    } catch (err) {
      toast("Deletion Failed", {
        description: "Failed to delete the GrabMart. Please try again.",
      });
    }
  };
  ...
  const handleResync = async (id, sync_type) => {
    try {
      const confirmSybc = await confirm({
        title: "Are you sure?",
        description: "This will Sync Data the GrabMart.",
      });
      ...
      toast("Sync Successful", {
        description: "The GrabMart has been successfully resynced.",
      });
    } catch (error) {
      toast("Sync Failed", {
        description: "Failed to resync the GrabMart. Please try again.",
      });
    }
  };
    ...
    <Card>
      ...
      <CardContent>
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="font-bold">GrabMart Code</TableHead>
              ...
              <TableRow>
                <TableCell colSpan={8} className="text-center">
                  <h2 className="py-5  text-gray-500 text-[16px]">
                    No GrabMart data found. Add your first GrabMart above.
                  </h2>
                </TableCell>
              </TableRow>
            ) : (
          ...
          <SheetHeader className="sticky top-0 bg-white z-[101] pb-4">
            <SheetTitle className="text-2xl font-bold text-black">
              <div className="flex w-full justify-between mt-4 gap-2">
                <div className="w-full">GrabMart Menu Sync</div>
